  ...
  assets: [
    {
      bytes: 12,
      id: "license",
      licenseRef: `${releasePath}/LICENSE-MediaPipe.txt`,
      path: `${releasePath}/LICENSE-MediaPipe.txt`,
...
describe("parseVisionManifest", () => {
  ...
  it.each([
    ...
    [
      "a path outside the pinned release directory",
      {
        ...validManifest,
    ...
    ["unsorted assets", { ...validManifest, assets: [...validManifest.assets].reverse() }],
    [
      "a non-HTTPS source",
      {
        ...validManifest,
        assets: [{ ...validManifest.assets[0]!, source: "http://example.test/a" }],
      },
    ],
    [
      "a non-release license path",
      {
        ...validManifest,
        assets: [{ ...validManifest.assets[0]!, licenseRef: "https://example.test/LICENSE" }],
      },
    ],
    [
      "an invalid hash",
    ...
    [
      ...
      { ...validManifest, assets: [{ ...validManifest.assets[0]!, unsafe: "value" }] },
    ],
    [
      "the removed selfie segmentation role",
      {
        ...validManifest,
        assets: [{ ...validManifest.assets[0]!, role: "selfie-segmentation-model" }],
      },
    ],
  ...
  it("finds an asset by its allowlisted role", () => {
    expect(getAssetByRole(validManifest, "wasm-loader-simd")).toEqual(validManifest.assets[1]!);
