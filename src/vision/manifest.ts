...
export const VISION_RELEASE_PATH_PREFIX = "/vision/mediapipe-0.10.35-face-landmarker-float16-v1/";
...
function isSameOriginPath(value: unknown): value is string {
  ...
  try {
    ...
    return (
      url.origin === SAME_ORIGIN && url.pathname === value && url.search === "" && url.hash === ""
    );
  } catch {
    return false;
  }
...
function isVisionReleasePath(value: unknown): value is string {
  return isSameOriginPath(value) && value.startsWith(VISION_RELEASE_PATH_PREFIX);
}
...
function isVisionAssetRole(value: unknown): value is VisionAssetRole {
  return typeof value === "string" && VISION_ASSET_ROLES.includes(value as VisionAssetRole);
}
...
export function parseVisionManifest(value: unknown): VisionReleaseManifest {
  if (!isValidManifest(value)) throw new Error("Invalid vision manifest");
  return value;
}
